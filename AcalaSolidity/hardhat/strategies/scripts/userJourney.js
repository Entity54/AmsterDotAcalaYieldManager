...
async function main() { 
  ...
  // console.log(`Transfering 2 ACA to the deployed smart contract`);
  // await primaryTokenInstance.transfer(`${instance.address}` , `${parseUnits(1,12)}`);
  // await primaryTokenInstance.transfer(instance.address , `100`);
  // console.log(`Tranfer of 2 AC to the deployed sc was successful`);
  // const sc_balnce_ACA = await primaryTokenInstance.balanceOf(instance.address);
  // console.log(`Smart contract bakance is : `,formatUnits(sc_balnce_ACA,12) , ` ACA`);
  ...
  await primaryTokenInstance.connect(initiator).transfer(instance.address, "3000000000000" );
  ...
  await instance.connect(initiator).initiateEscrow(beneficiaryAddress, ACA, 1000000000000, 10);
